...
export default function Step4Balance() {
    ...
    const aktivaGlow = balanced || difference > 0;
    const pasivaGlow = balanced || difference < 0;

    // Glow animation variants
    const glowAnimation = (isActive, color) => {
        if (!isActive) return {};
        return {
            boxShadow: [
                `0 0 20px ${color}40`,
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Left Column - Assets (AKTIVA) with Glow Effect */}
                <motion.div
                    className={`relative rounded-2xl transition-all duration-300 ${aktivaGlow
                            ? balanced
                                ? 'p-1 bg-gradient-to-br from-green-400 via-emerald-500 to-teal-400'
                                : 'p-1 bg-gradient-to-br from-blue-400 via-purple-500 to-indigo-500'
                            : 'p-0'
                        }`}
                    animate={glowAnimation(
                        aktivaGlow,
                        balanced ? 'rgba(16, 185, 129,' : 'rgba(99, 102, 241,'
                    )}
                    transition={{
                        duration: 2,
                        repeat: aktivaGlow ? Infinity : 0,
                        ease: 'easeInOut',
                    }}
                >
                    {/* Glow Label */}
                    {aktivaGlow && (
                        <motion.div
                            initial={{ opacity: 0, y: -10 }}
                            animate={{ opacity: 1, y: 0 }}
                            className={`absolute -top-3 left-4 z-10 px-3 py-1 rounded-full text-xs font-bold text-white shadow-lg ${balanced
                                    ? 'bg-gradient-to-r from-green-500 to-emerald-500'
                                    : 'bg-gradient-to-r from-blue-500 to-purple-500'
                                }`}
                        >
                            {balanced ? '✓ SEIMBANG' : '⬆ LEBIH BESAR'}
                        </motion.div>
                    )}

                    <div className={`bg-slate-50 dark:bg-slate-900 rounded-xl ${aktivaGlow ? 'p-4' : 'p-0'}`}>
                ...
                {/* Right Column - Liabilities & Equity (PASIVA) with Glow Effect */}
                <motion.div
                    className={`relative rounded-2xl transition-all duration-300 ${pasivaGlow
                            ? balanced
                                ? 'p-1 bg-gradient-to-br from-green-400 via-emerald-500 to-teal-400'
                                : 'p-1 bg-gradient-to-br from-red-400 via-rose-500 to-pink-500'
                            : 'p-0'
                        }`}
                    animate={glowAnimation(
                        pasivaGlow,
                        balanced ? 'rgba(16, 185, 129,' : 'rgba(244, 63, 94,'
                    )}
                    transition={{
                        duration: 2,
                        repeat: pasivaGlow ? Infinity : 0,
                        ease: 'easeInOut',
                    }}
                >
                    {/* Glow Label */}
                    ...
                    <div className={`bg-slate-50 dark:bg-slate-900 rounded-xl ${pasivaGlow ? 'p-4' : 'p-0'}`}>
                        <StaggerContainer className="space-y-4">
                            {/* Liabilities */}
                            <StaggerItem>
                                <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
